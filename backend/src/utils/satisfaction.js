...
const FIRST_OR_SUITE = new Set(['first', 'first_suite']);
...
/**
 * Evaluate one cabin class.
 * Returns { score, bevViolation, foodViolation, amenityViolated, sleepViolated,
 *           entertainmentViolated, lugViolation, seatViolation }
 * bevViolation / foodViolation / lugViolation = { have: number } or null.
 * seatViolation = 'seat_eco' | 'seat_biz' | 'seat_fir' | null.
 */
function evalCabin(cabin, items, seatType, distKm) {
  let score = 100;

  // Welcome Chocolate: flat +5 bonus, no penalty if missing
  if (items.has('Welcome Chocolate')) score += 5;

  // ── Beverages (3 independent threshold rules) ──────────────────────────────
  ...
  // ── Food (3 independent threshold rules) ───────────────────────────────────
  ...
  // ── Amenity Kit (per-class distance threshold) ─────────────────────────────
 ...
 * @param {object} params
 * @returns {{ score: number, violations: Array }}
 *
 * violations entry shapes:
 *   { rule: 'bev',  have: 0|1|2 }
 *   { rule: 'food', have: 0|1|2 }
 *   { rule: 'amenity' | 'sleep' | 'ent' | 'seat_eco' | 'seat_biz' | 'seat_fir' | 'maint' }
 *   { rule: 'lug',  have: 1|2 }   (1=cabin-only, 2=medium-only)
 */
function calcFlightSatisfaction(db, {
  ...
  // Fetch service items per cabin class from the assigned service profile
  ...
  if (serviceProfileId) {
    try {
      const stmt = db.prepare(`
        ...
        WHERE i.profile_id = ?
      `);
      stmt.bind([serviceProfileId]);
      while (stmt.step()) {
        const row = stmt.get();
        if (profileItems[row[0]]) profileItems[row[0]].add(row[1]);
      }
      stmt.free();
    } catch (e) { /* no items — all rules violated */ }
  }
  ...
  // Aggregate worst-case violations across cabins (one entry per rule type)
  let worstBev  = null;  // { have }
  let worstFood = null;  // { have }
  let worstLug  = null;  // { have }
  ...
  for (const cabin of ['economy', 'business', 'first']) {
    ...
    if (res.bevViolation) {
      bevCabins.push(cabin);
      if (!worstBev || res.bevViolation.have < worstBev.have) worstBev = res.bevViolation;
    }
    if (res.foodViolation) {
      foodCabins.push(cabin);
      if (!worstFood || res.foodViolation.have < worstFood.have) worstFood = res.foodViolation;
    }
    ...
    if (res.lugViolation) {
      lugCabins.push(cabin);
      if (!worstLug || res.lugViolation.have < worstLug.have) worstLug = res.lugViolation;
    }
  ...
  // Build violations list (ordered by impact, used for feedback display)
  const violations = [];
  ...
  if (worstLug) {
    // have: 1 = "has cabin only or nothing", 2 = "has medium only"
    violations.push({ rule: 'lug', have: Math.min(Math.max(worstLug.have, 1), 2), cabins: lugCabins });
  }
 ...
 * Falls back to 100 if no scored flights exist.
 */
function getAirlineSatisfactionScore(db, airlineId) {
  try {
    const stmt = db.prepare(`
...
/** Booking rate multiplier from airline average satisfaction score. */
