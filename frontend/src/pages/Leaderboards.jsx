...
const CATEGORIES = [
  { key: 'passengers',    label: 'Passengers',          valueKey: 'total_passengers',  unit: '' },
  { key: 'destinations',  label: 'Destinations',        valueKey: 'destination_count', unit: '' },
  { key: 'fleet',         label: 'Fleet Size',          valueKey: 'fleet_size',        unit: '' },
  { key: 'weekly_flights', label: 'Flights per Week',   valueKey: 'weekly_flights',    unit: '' },
];

function RankBadge({ rank }) {
  const medals = { 1: '🥇', 2: '🥈', 3: '🥉' };
  if (medals[rank]) return <span style={{ fontSize: '1.2rem' }}>{medals[rank]}</span>;
  return <span style={{ fontSize: '0.82rem', fontWeight: 700, color: '#999' }}>#{rank}</span>;
}
        ...
        {rows.map((row, i) => {
          ...
          return (
            <tr key={row.airline_id} style={{
              ...
              <td style={{ ...tdStyle, fontFamily: 'monospace', color: '#666', fontWeight: 600, letterSpacing: '0.04em' }}>{row.airline_code}</td>
              <td style={{ ...tdStyle, fontFamily: 'monospace', color: '#888' }}>{row.home_airport_code}</td>
  ...
  return (
    <div className="app">
      ...
      <div className="container" style={{ paddingTop: 24 }}>
        ...
        {/* KPI cards — your rank in each category */}
